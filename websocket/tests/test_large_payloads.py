...
class LargePayloadTest(unittest.TestCase):
        ...
        for size in test_sizes:
            with self.subTest(size=size):
                ...
                if size < ABNF.LENGTH_7:  # < 126
                    ...
                    expected_header_size = 2 + mask_size  # 1 byte opcode + 1 byte length + 4 byte mask
                elif size < ABNF.LENGTH_16:  # < 65536
                    # Length should be encoded in 2 bytes
                    expected_header_size = 4 + mask_size  # 1 byte opcode + 1 byte marker + 2 bytes length + 4 byte mask
                else:
                    # Length should be encoded in 8 bytes
                    expected_header_size = 10 + mask_size  # 1 byte opcode + 1 byte marker + 8 bytes length + 4 byte mask
    ...
    def test_utf8_validation_large_text(self):
        ...
        large_text_bytes = large_text.encode('utf-8')
