...
::
  ...
  python -m websocket._wsdump ws://websockets.chilkat.io/wsChilkatEcho.ashx -t "hello world"

The above command will provide you with an interactive terminal to communicate
with the echo server. This server will echo back any message you send it.
...
``python -m websocket._wsdump -h`` is seen below, showing the additional options available.

::

  python -m websocket._wsdump -h
  usage: _wsdump.py [-h] [-p PROXY] [-v [VERBOSE]] [-n] [-r]
                    [-s [SUBPROTOCOLS ...]] [-o ORIGIN] [--eof-wait EOF_WAIT]
                    [-t TEXT] [--timings] [--headers HEADERS]
                    ws_url

  WebSocket Simple Dump Tool

  positional arguments:
    ws_url                websocket url. ex.
